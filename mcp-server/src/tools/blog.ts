...
export function registerBlogTools(server: McpServer) {
  server.tool(
    "list_blog_posts",
    "List all blog posts with their frontmatter metadata",
    {},
    async () => {
  ...
  server.tool(
    "get_blog_post",
    "Get the full content and frontmatter of a blog post by slug",
    { slug: z.string().describe("Post slug — the filename without .mdx") },
    async ({ slug }) => {
      ...
      try {
        ...
        return {
          content: [{ type: "text" as const, text: `Post "${slug}" not found` }],
          isError: true,
        };
      }
    }
  ...
  server.tool(
    "create_blog_post",
    "Create a new MDX blog post. Fails if a post with the same slug already exists.",
    {
    ...
    async ({ slug, content, ...frontmatter }) => {
      ...
      try {
        ...
        return {
          content: [{ type: "text" as const, text: `Post "${slug}" already exists` }],
          isError: true,
        };
      } catch {
      ...
      return { content: [{ type: "text" as const, text: `Created ${slug}.mdx` }] };
    }
  );

  server.tool(
    "update_blog_post",
    "Update frontmatter and/or body of an existing blog post",
    {
      slug: z.string().describe("Slug of the post to update"),
      frontmatter: FrontmatterPatch.optional().describe(
        "Frontmatter fields to patch (partial — omitted fields are preserved)"
      ),
      content: z
        .string()
        .optional()
    ...
    async ({ slug, frontmatter: patch, content: newBody }) => {
      ...
      return { content: [{ type: "text" as const, text: `Updated ${slug}.mdx` }] };
    }
  );
}
